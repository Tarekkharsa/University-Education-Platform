...
function JoinAction({value, row}) {
  ...
  const bulkAction = () => {
    window.open(
      `${process.env.REACT_APP_API_URL}/bigBlueButton/joinAsModerator?password=${row.original.moderatorPW}&meeting_id=${row.original.meetingID}&fullname=${user.username}`,
    )
  }
  return (
    <>
